...
const HomeScreen = ({ navigation }) => {
  const renderGame = ({ item }) => (
    <TouchableOpacity
      className="flex-1 bg-gray-800 rounded-lg p-4 m-2 items-center"
      onPress={() => navigation.navigate('GamePlay', { game: item.name })}
    >
      <Image source={item.image} className="w-24 h-24 mb-2" resizeMode="contain" />
  ...
  return (
    <View className="flex-1 bg-gray-900 p-4">
      <View className="flex-row justify-between items-center mb-4">
        <Text className="text-white text-2xl font-semibold">GamingZone</Text>
      ...
      <Text className="text-white text-xl mb-4">Select a Game</Text>
      <FlatList
        data={games}
